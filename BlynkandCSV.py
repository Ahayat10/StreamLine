...
BLYNK_AUTH_TOKEN = 'YOURAUTHENTICATIONTOKEN'
...
def receiveData():
    ...
    if packet is not None:
        try:
            ...
            if 'x' in parsed_data:
                xyz_values = parsed_data['x'].split()
                x = float(xyz_values[0])
                # if 'Valve State' in parsed_data:
                # valve_state = parsed_data['Valve State']

            print("humidity:", humidity)
            print("Temp: ", temperature)
            print("Moisture:", moisture)
            print("x:", x)
            # print("valve_state:", valve_state)
            #blynk.virtual_write(4, moisture)
